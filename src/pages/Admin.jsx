...
// ─── Supabase Client ───────────────────────────────────────────────────────
// Vervang deze waarden met jouw eigen Supabase project URL en anon key
...
function OverviewTab({ cards, loading, onCopy, toast }) {
  ...
  return (
    <>
      ...
      {loading ? (
        <div style={{ textAlign: "center", color: "#6b8ab0", padding: "40px 0" }}>
          Laden...
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", color: "#6b8ab0", padding: "40px 0", fontSize: 13 }}>
          Geen kaarten gevonden
        </div>
      ) : (
        <div style={{ maxHeight: "50vh", overflowY: "auto" }}>
          {filtered.map(card => (
            <CodeRow key={card.id} card={card} onCopy={onCopy} />
          ))}
        </div>
      )}
    </>
...
function GenerateTab({ onGenerate, loading }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: "#6b8ab0", fontSize: 12, marginBottom: 20 }}>
        Codes worden direct opgeslagen in Supabase
      </div>

      <label style={{ color: "#8aa4c8", fontSize: 11, fontWeight: 700, letterSpacing: "0.5px" }}>
        AANTAL KAARTEN
      </label>
      ...
      <button
        onClick={() => onGenerate(count, value)}
        disabled={loading}
        style={{
          width: "100%",
...
function ValidateTab() {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: "#6b8ab0", fontSize: 12, marginBottom: 20 }}>
        Controleert live in Supabase database
      </div>
      ...
      {result && (
        <div style={{
          ...
          background: result.type === "valid" ? "rgba(0,212,170,0.1)" :
                      result.type === "redeemed" ? "rgba(0,212,170,0.08)" :
                      "rgba(255,80,80,0.1)",
          border: result.type === "valid" || result.type === "redeemed"
            ? "1px solid rgba(0,212,170,0.3)"
            : "1px solid rgba(255,80,80,0.3)",
        }}>
          {result.type === "valid" && (
            <>
              <div style={{ color: "#00d4aa", fontWeight: 800, fontSize: 14 }}>
                ✅ Geldig — SRD {result.card.value_srd}
              </div>
          ...
          {result.type === "redeemed" && (
            <div style={{ color: "#00d4aa", fontWeight: 800, fontSize: 14 }}>
              🎉 Succesvol inwisseld! SRD {result.card.value_srd} toegevoegd.
            </div>
          )}
          {result.type === "used" && (
            <div style={{ color: "#ff6b6b", fontWeight: 800, fontSize: 14 }}>
              ❌ Code al eerder gebruikt
            </div>
          )}
          {result.type === "expired" && (
            <div style={{ color: "#ff6b6b", fontWeight: 800, fontSize: 14 }}>
          ...
          {result.type === "notfound" && (
            <div style={{ color: "#ff6b6b", fontWeight: 800, fontSize: 14 }}>
              ❌ Code niet gevonden in database
            </div>
          )}
          {result.type === "error" && (
            <div style={{ color: "#ff6b6b", fontWeight: 800, fontSize: 14 }}>
              ⚠️ Fout bij verwerken — probeer opnieuw
            </div>
          )}
        </div>
      )}
...
export default function App() {
  ...
  // Generate batch and save to Supabase
  const handleGenerate = async (count, value) => {
    ...
    if (error) {
      showToast("Fout bij opslaan — controleer Supabase verbinding", "error");
      return;
    }
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(160deg, #080f1e 0%, #0d1f3c 60%, #080f1e 100%)",
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
          ...
          <button onClick={fetchCards} style={{
            ...
            color: "#8aa4c8", fontSize: 11, cursor: "pointer",
          }}>
            🔄 Vernieuwen
          </button>
        </div>
      ...
      <div style={{ padding: "20px" }}>
        {tab === "overview" && (
          <OverviewTab cards={cards} loading={loading} onCopy={handleCopy} toast={toast} />
        )}
        {tab === "generate" && (
          <GenerateTab onGenerate={handleGenerate} loading={loading} />
        )}
        {tab === "validate" && (
          <ValidateTab />
        )}
